...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., att_use = False):
        super().__init__()
        ...
        self.att_bias = nn.Linear(dim,dim * 2) if att_use else nn.Identity()
...
class Block(nn.Module):
    ...
    def __init__(
            ...
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, att_use = False):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, att_use = att_use)
    ...
    def forward(self, x, A_bias):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x),A_bias)))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block ,num_reg=0,A_use=False,att_use=False):
        """
        Args:
        ...
        self.A_bias = nn.Parameter(torch.zeros(1, depth, num_heads, num_patches + self.num_tokens + num_reg, num_patches + self.num_tokens + num_reg)) if A_use and att_use else None
        self.att_bias = nn.Parameter(torch.zeros(1, depth, num_heads, num_patches + self.num_tokens + num_reg, num_patches + self.num_tokens + num_reg)) if A_use and not att_use else None
        self.att_use = att_use
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, att_use = att_use)
    ...
    def forward_features(self, x):
        ...
        if self.att_bias is not None:
            att_bias = self.att_bias.expand(x.shape[0], -1, -1, -1, -1)
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            for blk in self.blocks:
                if self.A_bias is not None:
                    x = blk(x,A_bias[:,A])
                elif self.att_bias is not None:
                    x = blk(x,att_bias[:,A])
                else:
                    x = blk(x,None)
                A+=1
