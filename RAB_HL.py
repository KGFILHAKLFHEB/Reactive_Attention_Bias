...
class Attention(nn.Module):
    ...
    def forward(self, x, att,use_human=False):
        B, N, C = x.shape
        ...
        if use_human:
            if att is not None:
                ...
                # mask0 = mask==0
                att_bias = att_bias * mask
                # mask_bias = (att_bias * mask0).detach()
                # att_bias = (att_bias * mask) + (att_bias * mask0).detach()
                #print(att_bias.shape)
                # att_bias = attx_bias + mask_bias
                # print(att_bias.shape)
...
class Block(nn.Module):
    ...
    def forward(self, x, att,use_human=False):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x),att,use_human)))
