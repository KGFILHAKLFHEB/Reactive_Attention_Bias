...
class Attention(nn.Module):
    def __init__(self, token_num, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        super().__init__()
        ...
        self.att_bias = nn.Linear(dim, dim * 2)

    def forward(self, x, bias):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple)
        #att_bias = self.att_bias(x.detach()).reshape(B, N, self.num_heads, N).transpose(-3,-2)#.transpose(-2,-1))
        att_qk = self.att_bias(x.detach()).reshape(B, N, 2, self.num_heads, C//self.num_heads).permute(2, 0, 3, 1, 4)
        att_q, att_k = att_qk.unbind(0)
        att_bias = (att_q @ att_k.transpose(-2, -1)) * self.scale
        attn = (q @ k.transpose(-2, -1)) * self.scale
        # print(attn.shape)
        # print(att_bias.shape)
        attn = attn + att_bias*bias
...
class Block(nn.Module):
    ...
    def __init__(
            self,token_num, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., init_values=None,
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(token_num, dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x,bias):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x),bias)))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block ,num_reg=0,use_human=False):
        """
        Args:
        ...
        self.reg_token = nn.Parameter(torch.zeros(1, num_reg, embed_dim)) if self.num_reg > 0 else None
        self.pos_embed = nn.Parameter(torch.randn(1, num_patches + self.num_tokens + num_reg, embed_dim) * .02)
        self.A_bias = nn.Parameter(torch.zeros(1, depth, num_heads, num_patches + self.num_tokens + num_reg, num_patches + self.num_tokens + num_reg))
        self.pos_drop = nn.Dropout(p=drop_rate)

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        self.blocks = nn.Sequential(*[
            block_fn(
                token_num = num_patches + self.num_tokens + num_reg, dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, init_values=init_values,
    ...
    def forward_features(self, x, att = None):
        x = self.patch_embed(x)
        if self.cls_token is not None:
            x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1)
        if self.reg_token is not None:
            x = torch.cat((x, self.reg_token.expand(x.shape[0], -1, -1)), dim=1)
        x = self.pos_drop(x + self.pos_embed)
        if self.A_bias is not None:
            A_bias = self.A_bias.expand(x.shape[0],-1,-1,-1,-1)
            if self.use_human:
                if att is not None:
                    att = att.reshape(x.shape[0],196)
                    att = torch.cat((torch.ones(x.shape[0],1).to('cuda'),att),dim=1)
                    mask = att>0
                    mask = mask.reshape(x.shape[0],1,1,1,-1)
                    #mask1 = att==0
                    #mask1 = mask1.reshape(x.shape[0],1,1,1,-1)
                    A_bias = A_bias * mask
                    #A_bias2 = (A_bias * mask1).detach()
                    #A_bias = A_bias1 + A_bias2
